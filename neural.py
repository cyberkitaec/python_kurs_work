...
def resize_img(img):
    ...
    new_image = new_img.resize((1583, 891))
    new_image.show()
    new_image.save(img)
...
def predict_img(model, img):
    ...
    result_x = np.argmax(result, axis = 1)
    return chr(emnist_labels[result_x[0]])
...
def img_to_str(model: Any, image_file: str):
    ...
    #train_model(model)
    #model.save('text_reader_model.h5')

    model = keras.models.load_model('text_reader_model.h5')
   # resize_img("21.png")
    # s_out = img_to_str(model, "21.png")
   # print(s_out)
